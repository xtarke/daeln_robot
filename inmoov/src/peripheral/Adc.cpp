...
Adc::Adc() {
	...
	ADC_InitStructure.ADC_NbrOfChannel = N_CHANNELS;
	...
	/* PA_4 is channel 5 */
	ADC_RegularChannelConfig(ADC1, ADC_Channel_5, 4, ADC_SampleTime_28Cycles5);
	/* PA_6 is channel 6 */
...
uint16_t Adc::get_data(uint8_t channel){

	if (channel < N_CHANNELS)
		return ADC1ConvertedValues[channel];

	return 0;
}

void Adc::get_alldata(uint16_t *data){
